...
2. **Pin the Versions**: Read the project's manifest or lockfile when one exists, and say so when none does. Tie every version-dependent recommendation to the versions actually in use.
...
5. **Report**: Cite every claim with its source link. Cover the trade-offs and failure modes, not just the recommended option. Name any question you could not answer rather than implying it was settled, then close with actionable next steps tied to this project's versions and constraints.
6. **No Code / No Plan**: Do not modify code or draft implementation plans during the research phase. Create a file only when requested.
