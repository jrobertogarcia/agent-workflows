...
## Execution
1. **Scope**: Establish which suite, package, or directory is under audit. Ask if it is ambiguous. Reuse context already gathered in this thread rather than re-reading it.
2. **Audit**: Read the tests in scope. Judge each against F.I.R.S.T. (Fast, Independent, Repeatable, Self-Validating, Timely) and whether it asserts observable behavior rather than internal implementation, recording concrete instances of the anti-patterns above with file and line references. Report the absence of a problem as readily as its presence. Name any area you could not assess rather than implying it passed.
3. **Prioritize**: Group the findings into Quick Wins (low effort, high impact), Core Enhancements (medium effort, high impact), and Structural (high effort, high impact, including overall test shape). Omit any group with no findings.
4. **Stop for Approval**: Present the audit and the plan, then wait for approval. Do not rewrite tests, change assertions, or add coverage. Create a file only when requested.
